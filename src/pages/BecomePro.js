...
const BecomePro = () => {
  ...
  const steps = [
    ...
    { id: 5, title: 'Application Status', icon: CheckCircle, completed: currentStep > 5 }
  ];
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      ...
      case 4:
        ...
        break;
      case 5:
        if (!formData.professionalId.trim()) newErrors.professionalId = 'Professional ID is required';
        if (!formData.statusPassword.trim()) newErrors.statusPassword = 'Password is required';
        break;
      default:
        break;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentStep === 4 && validateStep(4)) {
      try {
        ...
        // Move to application status step
        setTimeout(() => {
          setCurrentStep(5);
        }, 2000);
      } catch (error) {
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-12 text-center">
            ...
            <p className="mt-4 text-lg text-gray-600 dark:text-gray-400">
              Complete the application below to start your journey with FixNow. We're excited to have you on board!
            </p>
          </div>
